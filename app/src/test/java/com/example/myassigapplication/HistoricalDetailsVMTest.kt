...
class HistoricalDetailsVMTest {
    ...
    @Before
    fun setUp(){
        `when`(fakeCurrencyRepository.getHistoricalRates(anyString(), anyString(), anyString())).thenReturn(JsonData.getExchangeRatesWithFlow())
        `when`(fakeCurrencyRepository.getTopCurrencyRates(anyString(), anyString(), anyString())).thenReturn(JsonData.getExchangeRatesWithFlow())
    ...
    fun testCurrencyList() {
        viewModel.onViewCreated("EUR", "INR")
